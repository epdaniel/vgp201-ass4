...
void Renderer::changeDirection(int dir) {
	float step = 0.004;
	switch (dir) {
		case GLFW_KEY_UP:
			moveDir = Eigen::Vector3f(0, step, 0);
			break;
		case GLFW_KEY_DOWN:
			moveDir = Eigen::Vector3f(0, -step, 0);
			break;
		case GLFW_KEY_LEFT:
			moveDir = Eigen::Vector3f(-step, 0, 0);
			break;
		case GLFW_KEY_RIGHT:
			moveDir = Eigen::Vector3f(step, 0, 0);
			break;
...
bool Renderer::checkCollisionHelper(Eigen::AlignedBox<double, 3> &box1, Eigen::AlignedBox<double, 3> &box2) {
	double R, R0, R1;
	...
	Eigen::RowVector3d A0 = scn->data_list[0].T.rotation().matrix().cast<double>() * Eigen::Vector3d(1, 0, 0);
	Eigen::RowVector3d A1 = scn->data_list[0].T.rotation().matrix().cast<double>() * Eigen::Vector3d(0, 1, 0);
	Eigen::RowVector3d A2 = scn->data_list[0].T.rotation().matrix().cast<double>() * Eigen::Vector3d(0, 0, 1);
	Eigen::Matrix3d A;
	A << A0[0], A1[0], A2[0],
		A0[1], A1[1], A2[1],
		A0[2], A1[2], A2[2];

	Eigen::RowVector3d B0 = scn->data_list[1].T.rotation().matrix().cast<double>() * Eigen::Vector3d(1, 0, 0);
	Eigen::RowVector3d B1 = scn->data_list[1].T.rotation().matrix().cast<double>() * Eigen::Vector3d(0, 1, 0);
	Eigen::RowVector3d B2 = scn->data_list[1].T.rotation().matrix().cast<double>() * Eigen::Vector3d(0, 0, 1);
	Eigen::Matrix3d B;
	B << B0[0], B1[0], B2[0],
		B0[1], B1[1], B2[1],
		B0[2], B1[2], B2[2];
...
bool Renderer::checkCollisionRec(igl::AABB<Eigen::MatrixXd, 3> *node1, igl::AABB<Eigen::MatrixXd, 3> *node2) {
	if (checkCollisionHelper(node1->m_box, node2->m_box))
	{
