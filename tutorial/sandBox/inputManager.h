...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case ':':
			...
			break;
		case GLFW_KEY_UP:
			rndr->changeDirection(GLFW_KEY_UP);
			break;
		case GLFW_KEY_DOWN:
			rndr->changeDirection(GLFW_KEY_DOWN);
			break;
		case GLFW_KEY_LEFT:
			rndr->changeDirection(GLFW_KEY_LEFT);
			break;
		case GLFW_KEY_RIGHT:
			rndr->changeDirection(GLFW_KEY_RIGHT);
			break;
		case ' ':
			rndr->toggleMove();
			break;
